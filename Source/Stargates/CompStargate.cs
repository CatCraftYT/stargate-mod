...
public class CompStargate : ThingComp
{
    ...
    private void GateDialTick()
    {
        ...
        if (targetMapParent is { HasMap: true } && TicksUntilOpen <= 220 && connectedGateSoundQueue == TicksUntilOpen && connectedGateSoundCounter < 3)
        {
            ...
            DefDatabase<SoundDef>.GetNamed($"StargateMod_ChevIncoming_{connectedGateSoundCounter + 1}").PlayOneShot(SoundInfo.InMap(connectedStargate));
            SgSoundDefOf.StargateMod_DhdUsual_1.PlayOneShot(SoundInfo.InMap(connectedStargate));
